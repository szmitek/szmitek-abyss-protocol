...
function migrateProfile(profile: UserProfile, history: WorkoutHistoryEntry[]): UserProfile {
  const legacy = profile as UserProfile & { attributeXp?: StatBlock; activeTrainingWeeks?: string[]; healthProfile?: UserProfile['healthProfile']; movementAssessments?: UserProfile['movementAssessments']; trainingArcs?: UserProfile['trainingArcs']; postureScans?: UserProfile['postureScans'] };
  ...
  return { ...profile, attributeXp, activeTrainingWeeks, healthProfile: legacy.healthProfile ?? { ...EMPTY_HEALTH_PROFILE }, movementAssessments: legacy.movementAssessments ?? [], trainingArcs: legacy.trainingArcs ?? [], postureScans: legacy.postureScans ?? [] };
}
...
function migrateSnapshot(parsed: StoredSnapshot): AppSnapshot {
  ...
  const lastCompletion = parsed.lastCompletion
    ...
    : null;
  return { ...INITIAL_SNAPSHOT, ...parsed, schemaVersion: 5, profile, history, lastCompletion };
}
...
export async function loadSnapshot(): Promise<AppSnapshot> {
  try {
    ...
    if (parsed.schemaVersion !== 1 && parsed.schemaVersion !== 2 && parsed.schemaVersion !== 3 && parsed.schemaVersion !== 4 && parsed.schemaVersion !== 5) return INITIAL_SNAPSHOT;
