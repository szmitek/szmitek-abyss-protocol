...
export function migrateSnapshot(parsed: StoredSnapshot): AppSnapshot {
  ...
  const lastCompletion = parsed.lastCompletion
    ...
    : null;
  return { ...INITIAL_SNAPSHOT, ...parsed, schemaVersion: 12, profile, history, lastCompletion, weeklyProtocol: parsed.weeklyProtocol ?? null, pendingArcReviewId: parsed.pendingArcReviewId ?? null };
}
