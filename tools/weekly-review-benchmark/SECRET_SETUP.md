...
The workflow must be reviewed and present on the default branch before GitHub exposes manual dispatch. PR46 is currently open; this preparation has not merged it, because the existing main-push Android workflow would otherwise start an unwanted APK build. Do not weaken the environment branch rule to run unmerged code. After setup confirmation, resolve the main-only workflow delivery without building an unchanged APK, then run only the secret check. A green secret check verifies injection, **not** key validity, expiry, permissions or billing; checking those by an OpenAI request still requires separate authorization.
