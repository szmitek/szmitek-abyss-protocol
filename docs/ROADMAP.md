...
3. Long-term programming: A/B/C objectives, weekly work/recovery distribution, missed sessions, return after interruption, lower-load periods and transitions between arcs. Readiness adapts the existing contract.
   ...
   - Current bounded package: explicit return-after-interruption mode, three marked training days followed by review, lower work limits, progression boundary and v15 storage/Vault compatibility. See `docs/RETURN_AFTER_BREAK.md` and `docs/HANDOFF.md` for delivery status.
